...
class PremiumWorkshopPdfResult {
  const PremiumWorkshopPdfResult({required this.bytes, required this.fileName});
...
class PremiumWorkshopPdfService {
  ...
  pw.Widget _buildFirstPage({
    ...
    return pw.Column(
      ...
      children: [
        ...
        _buildFirstPageHeader(locale: locale, request: request),
                              ...
                              children: [
                                pw.Expanded(
                                  child: _buildFirstPageCard(
                                    title: pdfT('client', locale),
                                    rows: [
                                      _PdfRow(
                                        label: pdfT('name', locale),
                                        value: _valueOrDash(
                                          request.customerName,
                                        ),
                                      ),
                                      _PdfRow(
                                        label: pdfT('phone', locale),
                                        value: _valueOrDash(
                                          request.customerPhone,
                                        ),
                                      ),
                                      _PdfRow(
                                        label: pdfT('email', locale),
                                        value: _valueOrDash(
                                ...
                                pw.Expanded(
                                  child: _buildFirstPageCard(
                                    title: pdfT('damage', locale),
                                    rows: [
                                      _PdfRow(
                                        label: pdfT('type', locale),
                                        value: _requestTitle(request, locale),
                                        maxLines: 2,
                                      ),
                                      _PdfRow(
                                        label: pdfT('location', locale),
                                        value: _valueOrDash(
                                          _damageTown(request),
                                        ),
                                      ),
                                      _PdfRow(
                                        label: _isOtherDamageRequest(request)
                                            ? pdfT('problemDate', locale)
                                            : pdfT('damageDate', locale),
                                        value: _valueOrDash(
                                          _damageDateLabel(request),
                                        ),
                                      ),
                                      _PdfRow(
                                        label: _isOtherDamageRequest(request)
                                            ? pdfT('problemTime', locale)
                                            : pdfT('damageTime', locale),
                                        value: _valueOrDash(
                                          _damageTime(request),
                                        ),
                                      ),
                                      _PdfRow(
                                        label: pdfT('description', locale),
                                        value: _valueOrDash(
                                          _damageDescription(request),
                                        ),
                                        maxLines: 2,
                                      ),
                                    ],
                          ...
                          pw.Expanded(
                            child: pw.Row(
                              ...
                              children: [
                                pw.Expanded(
                                  child: _buildFirstPageCard(
                                    title: pdfT('appointment', locale),
                                    rows: [
                                      _PdfRow(
                                        label: pdfT('day', locale),
                                        value: _formatDate(
                                          request.appointmentDate,
                                        ),
                                      ),
                                      _PdfRow(
                                        label: pdfT('time', locale),
                                        value: _appointmentTime(
                                          request.appointmentTime,
                                        ),
                                      ),
                                      _PdfRow(
                                        label: pdfT('workshop', locale),
                                ...
                                pw.Expanded(
                                  child: _buildFirstPageCard(
                                    title: pdfT('vehicleStatus', locale),
                                    rows: [
                                      _PdfRow(
                                        label: pdfT('currentKm', locale),
                                        value: _currentKmValue(
                                          locale,
                                          photoGroups,
                                        ),
                                      ),
                                      _PdfRow(
                                        label: pdfT('vehicleBrand', locale),
                                        value: _vehicleBrandValue(request),
                                      ),
                                    ],
                                  ),
                                ),
      ...
      pw.Positioned(
        ...
        child: pw.Opacity(
          ...
          child: pw.Transform.rotate(
            angle: -0.18,
            child: pw.Container(width: 2, height: 190, color: _accentOrange),
          ),
        ),
      ),
      pw.Positioned(
        ...
        child: pw.Opacity(
          ...
          child: pw.Transform.rotate(
            angle: -0.10,
            child: pw.Container(width: 1.2, height: 152, color: _accentOrange),
          ),
        ),
      ),
      pw.Positioned(
        ...
        child: pw.Opacity(
          ...
          child: pw.Transform.rotate(
            angle: 0.20,
            child: pw.Container(width: 2, height: 208, color: _accentOrange),
          ),
        ),
      ),
      pw.Positioned(
        ...
        child: pw.Opacity(
          ...
          child: pw.Transform.rotate(
            angle: 0.12,
            child: pw.Container(width: 1.2, height: 136, color: _accentOrange),
          ),
        ),
      ),
      pw.Positioned(
        ...
        child: pw.Opacity(
          ...
          child: pw.Transform.rotate(
            angle: -0.08,
            child: pw.Container(width: 1.2, height: 122, color: _accentOrange),
          ),
        ),
      ),
      pw.Positioned(
        ...
        child: pw.Opacity(
          ...
          child: pw.Transform.rotate(
            angle: 0.10,
            child: pw.Container(width: 1.2, height: 116, color: _accentOrange),
          ),
        ),
      ),
    ];
  ...
  pw.Widget _buildDocumentHeader({
    ...
    return pw.Column(
      ...
      children: [
        ...
        pw.Container(height: 1, color: const PdfColor(0.9686, 0.8745, 0.8157)),
      ],
    );
  }
    ...
    return pw.Container(
      ...
      child: pw.Column(
        ...
        children: [
          ...
          pw.SizedBox(height: 8),
          ..._buildFirstPageCardRows(rows: rows, columns: resolvedColumns),
        ],
      ),
    );
  }
    ...
    return pw.Column(
      ...
      children: [
        pw.Text(
          ...
          style: const pw.TextStyle(color: _textSecondary, fontSize: 7.8),
        ),
        pw.SizedBox(height: 2),
        pw.Text(
          row.value,
        ...
        children: [
          ...
          pw.Row(
            ...
            children: [
              pw.Expanded(
                child: pw.Text(
                  pdfT('professionalFooter', locale),
                  style: const pw.TextStyle(color: _textSecondary, fontSize: 8),
                ),
              ),
              pw.Expanded(
                child: pw.Text(
                  '${pdfT('generatedOn', locale)}: $generatedAt',
              ...
              pw.Expanded(
                child: pw.Text(
                  '${pdfT('page', locale)} ${context.pageNumber} ${pdfT('of', locale)} ${context.pagesCount}',
    ...
    return pw.Container(
      ...
      child: pw.Column(
        ...
        children: [
          ...
          pw.SizedBox(height: 18),
          _buildQrMetaRow(
            label: pdfT('caseNumber', locale),
            value: _referenceNumber(request.id),
          ),
          pw.SizedBox(height: 10),
          _buildQrMetaRow(
            label: pdfT('generatedOn', locale),
            value: generatedAt,
          ),
          pw.SizedBox(height: 10),
          _buildQrMetaRow(
            label: pdfT('uniqueReference', locale),
            value: request.id,
          ),
        ],
      ),
  ...
  pw.Widget _buildQrMetaRow({required String label, required String value}) {
    return pw.Column(
      crossAxisAlignment: pw.CrossAxisAlignment.start,
      children: [
        pw.Text(
  ...
  Future<Map<String, pw.MemoryImage>> _loadPhotoRegistry(
    ...
    final sources = <String>{for (final group in groups) ...group.sources};
    await Future.wait(
  ...
  String _appointmentWorkshopValue(String locale, String? workshopLabel) {
    ...
    return pdfT('partnerWorkshop', locale);
  }
  ...
  String _currentKmValue(String locale, List<_PhotoGroup> photoGroups) {
    ...
    if (count == 0) return '-';
    return pdfT('seePhoto', locale, params: {'count': '$count'});
  }

  String _vehicleBrandValue(AppointmentRequest request) {
    return '-';
  }
  ...
  String _statusLabel(String locale, String status) {
    switch (status) {
      case 'confirmed':
        return pdfT('appointmentConfirmed', locale);
      case 'in_progress':
        return pdfT('inProgress', locale);
      case 'completed':
        return pdfT('repairCompleted', locale);
      case 'cancelled':
        return pdfT('appointmentCancelled', locale);
      default:
        return pdfT('requestSent', locale);
    }
  }
  ...
  String pdfT(String key, String lang, {Map<String, String>? params}) {
    final locale = _normalizeLocale(lang);
    final translations = _pdfTranslations[key];
    var value = translations?[locale] ?? translations?['de'] ?? key;
    if (params != null) {
  ...
  static const Map<String, Map<String, String>> _pdfTranslations = {
    'requestSent': {
      'de': 'Anfrage gesendet',
    ...
    'professionalFooter': {
      ...
      'en': 'CrashForm - Professional damage report',
    ...
    'digitalReport': {
      'de': 'Digitale Schadendokumentation',
      'it': 'Documentazione sinistri digitale',
      'en': 'Digital damage documentation',
      'fr': 'Documentation numerique des dommages',
    },
    'claimVerification': {
    ...
    'scanQrDetails': {
      'de':
          'Scannen Sie den QR-Code, um auf die Details der Anfrage zuzugreifen.',
...
class _PdfRow {
  const _PdfRow({required this.label, this.value = '', this.maxLines});
...
class _PhotoGroup {
  const _PhotoGroup({required this.title, required this.sources});
...
class _PhotoPageEntry {
  const _PhotoPageEntry({required this.title, required this.image});
